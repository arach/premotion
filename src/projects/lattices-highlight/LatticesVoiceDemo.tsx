...
const CLIPS: Clip[] = [
	{
		...
		sublabel: "Hands-off mode engaged",
		startFrom: 155,
		duration: 10,
	},
	{
		label: "AGENT RESPONSE",
		sublabel: "Multi-pane terminal output",
		startFrom: 167,
		duration: 12,
	},
	{
		label: "COMMAND EXECUTION",
		sublabel: "Window management via voice",
		startFrom: 182,
		duration: 12,
	},
];
...
// ── Crop settings ────────────────────────────────────────
// Source: 3440x1440 ultrawide. Voice demo shows full terminal layout.
// Wider crop to capture all the terminal panes.
const CROP_SCALE = 1.2;
const CROP_CENTER_X = 0.5; // center — voice demo uses full width
const INNER_SCALE_X = 0.78;
const INNER_SCALE_Y = 0.68;
...
const PromptCard: React.FC = () => {
	...
	const promptText =
		"Demonstrate hands-off voice mode — speak commands to Lattices and watch it orchestrate windows, layouts, and agents without touching the keyboard.";
			...
			>
				...
				>
					<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
						<span style={{ color: dotVisible ? "#f97316" : "#444", fontSize: 8 }}>
							●
						</span>
						<span
							style={{
...
export interface LatticesVoiceDemoProps {
	videoSrc?: string;
	musicTrack?: string;
	musicVolume?: number;
	title?: string;
...
export const LatticesVoiceDemo: React.FC<LatticesVoiceDemoProps> = ({
...
export function calculateVoiceDemoFrames(fps: number): number {
	const introFrames = Math.floor(INTRO_DURATION * fps);
	const promptFrames = Math.floor(PROMPT_DURATION * fps);
	const outroFrames = Math.floor(OUTRO_DURATION * fps);
