...
// Curated clips from the 2-minute agent coordination update
const CLIPS: Clip[] = [
	{
		label: "STATUS UPDATE",
		sublabel: "Voice pipeline hardening",
		startFrom: 2,
		duration: 28,
	},
	{
		label: "QUESTION",
		sublabel: "Clarifying ownership",
		startFrom: 29,
		duration: 5,
	},
	{
		label: "COORDINATION",
		sublabel: "Lattices + Talkie pipeline",
		startFrom: 65,
		duration: 14,
	},
	{
		label: "RESOLUTION",
		sublabel: "Agents fully coordinated",
		startFrom: 104,
		duration: 19,
	},
];
...
const PromptCard: React.FC = () => {
	...
	const promptText =
		"Multi-agent coordination — Lattices, Talkie, and Vox hardening the voice pipeline together.";
...
export interface LatticesVoiceSessionBProps {
	videoSrc?: string;
...
export const LatticesVoiceSessionB: React.FC<LatticesVoiceSessionBProps> = ({
...
export function calculateVoiceSessionBFrames(fps: number): number {
	const introFrames = Math.floor(INTRO_DURATION * fps);
	const promptFrames = Math.floor(PROMPT_DURATION * fps);
	const outroFrames = Math.floor(OUTRO_DURATION * fps);
