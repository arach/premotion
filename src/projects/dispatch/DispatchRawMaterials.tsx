...
// ── Dispatch logo dot map ───────────────────────────────────────────
// Extracted from dispatch slashgrid SVG: 24x24 grid, 512x512 canvas
// Grid starts at 29.8px, spacing 19.67px, dot radius 5.9px
// 153 amber logo dots, 423 gray background dots
const GRID_SIZE = 24;
const GRID_START = 29.8;
const GRID_STEP = 19.67;
const DOT_R = 5.9;
...
export const DispatchRawDotBuild: React.FC = () => {
	...
	for (let row = 0; row < GRID_SIZE; row++) {
		for (let col = 0; col < GRID_SIZE; col++) {
			...
			circles.push(
				<circle key={`bg-${col}-${row}`} cx={dotCx(col)} cy={dotCy(row)} r={DOT_R} fill={BG_DOT} opacity={0.08} />,
			);
		}
	}
...
export const DispatchRawDotIdle: React.FC = () => {
	const frame = useCurrentFrame();
...
export const DispatchRawDotDissolve: React.FC = () => {
	const frame = useCurrentFrame();
...
export const DispatchRawScanPass: React.FC = () => {
	const frame = useCurrentFrame();
	...
	return (
		<AbsoluteFill style={{ backgroundColor: BG }}>
			<Img src={staticFile("dispatch-logo.svg")} style={{ width: "100%", height: "100%" }} />
			<ScanEffect
				direction="vertical"
				progress={progress}
				intensity={intensity}
...
export const DispatchRawScanHorizontal: React.FC = () => {
	const frame = useCurrentFrame();
...
export const DispatchRawLogoIn: React.FC = () => {
	const frame = useCurrentFrame();
	...
	return (
		<AbsoluteFill style={{ backgroundColor: BG }}>
			<Img src={staticFile("dispatch-logo.svg")} style={{ width: "100%", height: "100%", opacity }} />
		</AbsoluteFill>
	);
};
...
export const DispatchRawLogoOut: React.FC = () => {
	const frame = useCurrentFrame();
...
export const DispatchRawGlowPulse: React.FC = () => {
	const frame = useCurrentFrame();
		...
		<AbsoluteFill style={{ backgroundColor: BG }}>
			<div
				style={{
					...
					inset: 0,
					borderRadius: 10,
					background: `radial-gradient(circle at 50% 50%, rgba(255, 159, 41, ${glow}) 0%, rgba(255, 159, 41, ${glow * 0.3}) 40%, transparent 70%)`,
				}}
			/>
		</AbsoluteFill>
	);
